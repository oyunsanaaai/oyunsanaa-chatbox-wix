(() => {
  ...
  function applyTheme(i) {
    document.documentElement.setAttribute('data-t', i);
  }
  ...
  const scrollBottom = () => {
    el.stream.scrollTop = el.stream.scrollHeight + 999;
  };
  ...
  (function redraw() {
    ...
    const arr = loadMsgs();
    if (!arr.length) {
      bubble('Сайн уу! Оюунсанаатай ярилцъя. 🌿', 'bot');
  ...
  function showTyping() {
    if (el.typing) el.typing.hidden = false;
  }
  function hideTyping() {
    if (el.typing) el.typing.hidden = true;
  }
  ...
  function renderPreviews() {
    if (!previewImages.length) {
      el.previews.hidden = true;
      el.previews.innerHTML = '';
      return;
    }
    el.previews.hidden = false;
    el.previews.innerHTML = previewImages
      .map(
        (d, i) =>
  ...
  async function callChat({ text = '', images = [] }) {
    if (!OY_API) {
      bubble('⚠️ API тохируулга хийгдээгүй (OY_API_BASE).', 'bot');
      return;
    }
    showTyping();
    try {
      ...
      const reply =
        j?.output?.[0]?.content?.find?.(c => c.type === 'output_text')?.text || j?.reply || '…';
      bubble(reply, 'bot');
      pushMsg('bot', reply);
  ...
  document.querySelectorAll('.oy-item[data-menu]').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const target = Array.from(document.querySelectorAll('.oy-pane')).find(
        p => p.dataset.pane === key
      );
      if (!target) return;
      if (!target.hidden) {
        target.hidden = true;
