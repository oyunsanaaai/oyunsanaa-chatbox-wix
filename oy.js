...
(function(){
  // ЭНД линкээ тавь:
  const SITE_SIGNUP = 'https://oyunsanaa.com/signup';
  const SITE_FORGOT = 'https://oyunsanaa.com/forgot';

  const $ = s => document.querySelector(s);
  const loginWrap = $('#oyLogin');

  // Нэвтэрсэн эсэх (demo)
  const isIn = () => localStorage.getItem('oy_session') === '1';

  // Чат ачаалахаас өмнө—нэвтрээгүй бол логин гаргана
  if (!isIn()) loginWrap.hidden = false;

  // Товч/линк
  $('#oySignupBtn').onclick = () => location.href = SITE_SIGNUP;
  $('#oyForgot').onclick = (e)=>{ e.preventDefault(); location.href = SITE_FORGOT; };

  // Нэвтрэх
  $('#oyLoginBtn').onclick = () => {
    const u = $('#oyUser').value.trim();
    const p = $('#oyPass').value.trim();
    if (!u || !p) { alert('И-мэйл/утас, нууц үг хоёроо бөглөнө үү.'); return; }

    // ЖИНХЭНЭ баталгаажуулалтыг хожим энд хийнэ (fetch('/api/auth', ...))
    // Одоохондоо demo төлөв:
    localStorage.setItem('oy_session','1');

    // Логин цонхоо хааж чатыг харагдуулах
    loginWrap.hidden = true;

    // Хэрэв чатыг эхлүүлэх тусгай функцтай бол:
    if (typeof window.oyBootChat === 'function') window.oyBootChat();
  };

  // Гарахыг хүсвэл: window.oyLogout = () => { localStorage.removeItem('oy_session'); location.reload(); };
})();
</script>
